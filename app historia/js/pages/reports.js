...
const ReportsPage = {
    ...
    async generateFilteredReport() {
        ...
        let title = 'Reporte de Consultas';
        if (species) title += ` (${species})`;
        if (startDate || endDate) title += ` [${startDate} - ${endDate}]`;

        const contentHtml = await this.generateConsultationsContent(filteredConsultations);
    ...
    generatePatientsContent(filteredData = null) {
